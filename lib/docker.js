...
function buildDockerImage() {
    ...
    const baseDockerImage = "public.ecr.aws/lambda/python:3.12"
    const LAMBDA_TASK_ROOT = `/var/task/${this.serverless.service.service}/`
    //"amazon/aws-lambda-python:3.9.2021.08.23.12"; //justinram11/lambda:${this.serverless.service.provider.runtime}
    ...
    // FROM public.ecr.aws/lambda/${this.serverless.service.provider.runtime} AS builder
    ...
    #COPY pythonRequirements.zip /tmp
    
    RUN dnf install -y git tar make gcc g++

    RUN cd /tmp && tar -zxf ${this.serverless.service.service}.zip && rm ${this.serverless.service.service}.zip
    #RUN cd /tmp && tar -zxf pythonRequirements.zip && rm pythonRequirements.zip
    
    FROM ${baseDockerImage}
    COPY --from=builder /tmp /var/task/${this.serverless.service.service}/
    RUN rm -rf /tmp/*
    RUN ls -la /var/task/
    RUN ls -la /var/task/${this.serverless.service.service}/

    # Start of custom code for data_ml_serverless repo

    # Copy necessary files to the Lambda task root.
    COPY Pipfile ${LAMBDA_TASK_ROOT}/
    COPY Pipfile.lock ${LAMBDA_TASK_ROOT}/
    COPY src/ ${LAMBDA_TASK_ROOT}/src
    COPY scanbuy_cluster.yml ${LAMBDA_TASK_ROOT}/scanbuy_cluster.yml

    # Set up the Python environment.
    RUN pip install pipenv
    RUN pipenv install --system --deploy --ignore-pipfile --python 3.12

    # Set the CMD to your handler (could also be done as a parameter override outside of the Dockerfile)
    CMD [ "src/functions/new_pipeline.feature_selection_handler" ]
    `;
