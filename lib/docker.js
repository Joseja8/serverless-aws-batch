...
function buildDockerImage() {
    ...
    const dockerFileContents =
        `
        FROM python:3.12
        LABEL maintainer="Jose Javier Rosell (josejavier.rosell@scanbuy.com)"
        LABEL app="data-ml-pipeline"
        LABEL product="data"
        LABEL sb_managed_by="serverless"
        LABEL sb_repo="data_ml_serverless"
        
        COPY ${this.serverless.service.service}.zip .
        
        RUN unzip -q ${this.serverless.service.service}.zip
        RUN rm ${this.serverless.service.service}.zip
        
        ENTRYPOINT [ "python", "-m", "src.libs.ml.feature_selection" ]
        `;
