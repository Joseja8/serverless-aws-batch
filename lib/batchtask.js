...
function compileBatchTask(functionName) {
  ...
  // Merge provider-level batch tags with per-function tags
  const batchTags = Object.assign(
    {},
    this.serverless.service.provider.batch.Tags || {},
    functionObject.tags || {},
  );
  ...
  // Note: NetworkConfiguration.AssignPublicIp is set in updateContainerProperties()
    ...
    [functionName]: {
      ...
      environment: {
        ...
        TAG_TEAM: _.get(
          functionObject,
          "tags.team",
          _.get(this.serverless.service, "provider.tags.team", ""),
        ),
        TAG_PROJECT: _.get(
          functionObject,
          "tags.project",
          _.get(this.serverless.service, "provider.tags.project", ""),
        ),
        TAG_ENV: _.get(
          functionObject,
          "tags.env",
          _.get(this.serverless.service, "provider.tags.env", ""),
        ),
        TAG_PIPELINE: _.get(functionObject, "tags.pipeline", ""),
        TAG_LAMBDA: _.get(functionObject, "tags.lambda", functionName),
        BATCH_EXECUTION_TAGS: JSON.stringify(
          getExecutionTagsConfig(this.serverless.service, functionObject),
        ),
        BATCH_JOB_NAME_SUFFIXES: JSON.stringify(
          getJobNameSuffixesConfig(this.serverless.service, functionObject),
        ),
      },
      role: {
        "Fn::GetAtt": [
...
function updateContainerProperties(newFunction, functionObject, functionName) {
  ...
  if (!newFunction.Properties.ContainerProperties.hasOwnProperty("Command")) {
    if (!functionObject.handler) {
      newFunction.Properties.ContainerProperties.Command = ["Ref::event"];
    } else {
  ...
  if (!newFunction.Properties.ContainerProperties.hasOwnProperty("Image")) {
    newFunction.Properties.ContainerProperties.Image =
  ...
  // if (! newFunction.Properties.ContainerProperties.hasOwnProperty('Volumes')) {
  //   newFunction.Properties.ContainerProperties.Vcpus = 1;
  // }
  // if (! newFunction.Properties.ContainerProperties.hasOwnProperty('MountPoints')) {
  //   newFunction.Properties.ContainerProperties.Vcpus = 1;
  // }

  // Set networkConfiguration->assignPublicIp to ENABLED
  _.set(
    newFunction,
  ...
  const environment = _.merge(
    {
      ...
      PIPELINE: _.get(functionObject, "tags.pipeline", ""),
      LAMBDA: _.get(functionObject, "tags.lambda", functionName),
    },
...
function getProviderBatchConfig(
  serverlessService,
  upperKey,
  lowerKey,
  fallback,
) {
  const providerBatch = _.get(serverlessService, "provider.batch", {});
  return _.get(
    providerBatch,
    upperKey,
    _.get(providerBatch, lowerKey, fallback),
  );
}

function getFunctionBatchConfig(functionObject, upperKey, lowerKey, fallback) {
  return _.get(
    functionObject,
    `batch.${upperKey}`,
    _.get(functionObject, `batch.${lowerKey}`, fallback),
  );
}

function getExecutionTagsConfig(serverlessService, functionObject) {
...
function getJobNameSuffixesConfig(serverlessService, functionObject) {
  return [
    ...toArray(
      getProviderBatchConfig(
        serverlessService,
...
function generateBatchJobExecutionRole(functionName, functionObject) {
  const batchJobExecutionRoleName =
    `BatchJobRole-${functionName}-${this.provider.getRegion()}-${this.provider.getStage()}-${this.provider.serverless.service.service}`.substring(
      0,
      64,
    );
  ...
  return {
    [this.provider.naming.getBatchJobExecutionRoleLogicalId(functionName)]:
