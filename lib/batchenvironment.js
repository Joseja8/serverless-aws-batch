...
const GENERATED_NAME_VISIBLE_PREFIX_LENGTH = 24;
const BATCH_COMPUTE_ENVIRONMENT_LOGICAL_ID_SUFFIX = "BatchCompute";
const BATCH_JOB_QUEUE_LOGICAL_ID_SUFFIX = "BatchQueue";
...
/**
 * @returns {string} The CloudFormation-safe Batch resource logical ID.
 */
function getBatchResourceLogicalId(suffix) {
  const maxPrefixLength = GENERATED_NAME_VISIBLE_PREFIX_LENGTH - suffix.length;
  const prefix = getServiceStageLogicalIdPrefix.call(this, maxPrefixLength);

  return `${prefix}${suffix}`;
}

function getServiceStageLogicalIdPrefix(maxLength) {
  const provider = this.provider || this;
  const tokens = [
    ...getCloudFormationIdParts(provider.serverless.service.service),
    ...getCloudFormationIdParts(provider.getStage()),
  ];
  const prefix = compactLogicalIdPrefix(tokens, Math.max(maxLength, 0));

  return /^[A-Za-z]/.test(prefix)
    ? prefix
    : `Svc${prefix}`.substring(0, maxLength);
}

function compactLogicalIdPrefix(tokens, maxLength) {
  const parts = tokens.map(toCloudFormationIdPart);

  while (parts.join("").length > maxLength) {
    const index = parts.findIndex((part) => part.length > 3);
    if (index === -1) {
      break;
    }
    parts[index] = abbreviateCloudFormationIdPart(parts[index]);
  }

  return parts.join("").substring(0, maxLength);
}

function getCloudFormationIdParts(value) {
  return String(value || "").match(/[A-Za-z0-9]+/g) || [];
}

function toCloudFormationIdPart(value) {
  return `${value.charAt(0).toUpperCase()}${value.slice(1)}`;
}

function abbreviateCloudFormationIdPart(part) {
  const consonant = part.slice(1).match(/[B-DF-HJ-NP-TV-Zb-df-hj-np-tv-z]/);

  return `${part.charAt(0)}${consonant ? consonant[0] : part.charAt(1)}`;
}

/**
 * @returns {string} A service/stage-scoped Batch compute environment logical ID
 */
function getBatchComputeEnvironmentLogicalId() {
  return getBatchResourceLogicalId.call(
    this,
    BATCH_COMPUTE_ENVIRONMENT_LOGICAL_ID_SUFFIX,
  );
}

/**
 * @returns {string} A service/stage-scoped Batch job queue logical ID
 */
function getBatchJobQueueLogicalId() {
  return getBatchResourceLogicalId.call(
    this,
    BATCH_JOB_QUEUE_LOGICAL_ID_SUFFIX,
  );
}
...
function getBatchJobQueueName() {
  ...
  return `${provider.serverless.service.service}-${provider.getStage()}-JobQueue`;
}
...
function generateBatchServiceRole() {
  const batchServiceRoleName =
    `BatchServiceRole-${this.provider.getRegion()}-${this.provider.getStage()}-${this.provider.serverless.service.service}`.substring(
      0,
      64,
    );
...
function generateBatchInstanceRole() {
  const batchInstanceRoleName =
    `BatchInstanceRole-${this.provider.getRegion()}-${this.provider.getStage()}-${this.provider.serverless.service.service}`.substring(
      0,
      64,
    );
  ...
  return {
    [this.provider.naming.getBatchInstanceManagementRoleLogicalId()]:
      JSON.parse(batchInstanceManagementRoleTemplate),
    [this.provider.naming.getBatchInstanceManagementProfileLogicalId()]:
      JSON.parse(batchInstanceProfileTemplate),
  };
}
...
function generateBatchSpotFleetRole() {
  const batchSpotRoleName =
    `BatchSpotFleetRole-${this.provider.getRegion()}-${this.provider.getStage()}-${this.provider.serverless.service.service}`.substring(
      0,
      64,
    );
  ...
  return {
    [this.provider.naming.getBatchSpotFleetManagementRoleLogicalId()]:
      JSON.parse(batchSpotRoleManagementTemplate),
  };
}
...
function generateLambdaScheduleExecutionRole() {
  const lambdaScheduleExecutionRoleName =
    `BatchScheduleRole-${this.provider.getRegion()}-${this.provider.getStage()}-${this.provider.serverless.service.service}`.substring(
      0,
      64,
    );
  ...
  return {
    [this.provider.naming.getLambdaScheduleExecutionRoleLogicalId()]:
      JSON.parse(lambdaScheduleExecutionRoleTemplate),
  };
}
...
function generateBatchComputeEnvironment() {
  ...
  if (this.serverless.service.provider.hasOwnProperty("batch")) {
    if (
      this.serverless.service.provider.batch.hasOwnProperty("SecurityGroupIds")
    ) {
      batchComputeResourceObject["SecurityGroupIds"] =
        this.serverless.service.provider.batch.SecurityGroupIds;
    }
    if (this.serverless.service.provider.batch.hasOwnProperty("Subnets")) {
      batchComputeResourceObject["Subnets"] =
        this.serverless.service.provider.batch.Subnets;
    }
  }
  ...
  return {
    [this.provider.naming.getBatchComputeEnvironmentLogicalId()]:
      computeEnvironment,
  };
}
...
function generateAWSBatchTemplate() {
  ...
  const newBatchInstanceManagementRoleObject =
    generateBatchInstanceRole.bind(this)();
  const newBatchSpotFleetManagementObject =
    generateBatchSpotFleetRole.bind(this)();
  const newLambdaScheduleExecutionRoleObject =
    generateLambdaScheduleExecutionRole.bind(this)();
  const newBatchJobQueueObject = generateBatchJobQueue.bind(this)();
  const newBatchComputeEnvironmentObject =
    generateBatchComputeEnvironment.bind(this)();
