...
function generateBatchJobQueue() {
    ...
    // Append a random suffix so CloudFormation can replace the resource without name collisions.
    const suffix = Math.floor(Math.random() * 1000000);
    jobQueue.Properties.JobQueueName = `${jobQueue.Properties.JobQueueName}-${suffix}`.substring(0, 128);
...
function generateBatchComputeEnvironment() {
    ...
    const suffix = Math.floor(Math.random() * 1000000);
    const computeEnvironmentName = `${this.provider.serverless.service.service}-${this.provider.getStage()}-ComputeEnvironment-${suffix}`.substring(0, 128);
    ...
    // Apply tags at the ComputeEnvironment level (not inside ComputeResources)
    if (Object.keys(computeEnvironmentTags).length > 0) {
