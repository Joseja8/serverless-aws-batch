...
module.exports.schedule = (event, context, callback) => {
    ...
    // Optional context segments (ea_name, chunk indices).
    let context = '';

    if (event.ea_name) {
        context += '-' + event.ea_name;
    }
    ...
    const sanitised = (baseName + context).replace(/[^A-Za-z0-9_-]/g, '').slice(0, MAX_JOB_NAME_LENGTH - suffix.length);
